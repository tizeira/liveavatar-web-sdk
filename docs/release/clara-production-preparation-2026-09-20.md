...
Validación aislada completada el 21/09/2026 en la rama schema-only de Neon `test/clara-catalog-2026-09-21` (`br-calm-lake-a4v9dybs`, vencimiento automático 24/09). Se aplicó únicamente la migración `20260922010000_add_clara_catalog`; pasaron el reemplazo transaccional, búsqueda, intercambio de handles y catálogo vacío contra Postgres real. La sincronización Shopify → Neon persistió 11 productos reales, el estado registró el mismo conteo y una búsqueda sobre el catálogo sincronizado devolvió el producto esperado. No se tocó `main`, no se creó webhook, no se modificó ElevenLabs y no se desplegó esta rama.
